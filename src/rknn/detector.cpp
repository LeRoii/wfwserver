...
int detector()
{
  ...
  printf("Read %s ...\n", image_name);
  cv::Mat orig_img = cv::imread("/home/fex/detrknn/1.jpg", 1);
  if (!orig_img.data) {
  ...
  unsigned char* model_data      = load_model("/home/fex/detrknn/det.rknn", &model_data_size);
...
//   printf("img width = %d, img height = %d\n", img_width, img_height);
  ...
  printf("Loading mode...\n");
  int            model_data_size = 0;
  unsigned char* model_data      = load_model("/app/det.rknn", &model_data_size);
  ret                            = rknn_init(&ctx, model_data, model_data_size, 0, NULL);
  if (ret < 0) {
    printf("rknn_init error ret=%d\n", ret);
    return -1;
...
int  DetectorRun(cv::Mat &img, std::vector<std::string> &detret)
{
    ...
    // post process
    float scale_w = 1;//(float)width / img_width;
    float scale_h = 1;//(float)height / img_height;
    ...
    {
      detret.push_back("person");
      detret.push_back("x1:" + std::to_string(x1) + "y1:" + std::to_string(y1) + "x2:" + std::to_string(x1) + "y2:" + std::to_string(y2));
    }
    else if(strcmp(det_result->name, "car") == 0
            || strcmp(det_result->name, "truck") == 0)
    {
      detret.push_back("vehicle");
      detret.push_back("x1:" + std::to_string(x1) + "y1:" + std::to_string(y1) + "x2:" + std::to_string(x1) + "y2:" + std::to_string(y2));
    }
    }
